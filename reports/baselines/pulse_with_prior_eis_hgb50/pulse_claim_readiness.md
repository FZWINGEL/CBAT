...
| C-rate resistance prediction | supported_for_scalar_diagnostics | Best C-rate row `L2_hist_gradient_boosting + P3_state_nominal` has condition-mean MAE `0.00193709`. | Use as scalar baseline evidence only. |
| Stress-feature contribution | partially_supported | `P5_stress_v1_1` appears in `5` primary leaderboard rows. | Useful in condition/temperature views, not uniformly C-rate. |
