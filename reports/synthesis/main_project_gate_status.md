...
| Stressor-robust capacity | Narrow replicated adaptive diagnostic supported / stressor-family routing diagnostic supported / target transfer not supported / capacity-level reconstruction branch closed / broad solved-fade claim blocked | Fixed-weight stressor-balanced HGB improves C-rate delta but the predeclared R2/F8/w1 setting fails outside-C-rate degradation at `0.0528343`; conservative train-only adaptive R2/F8 selection replicates across five logical deterministic seeds with C-rate gains `0.0200436` vs F4 and `0.0214266` vs stress R0, paired p05 above zero, and max outside-C-rate degradation `0.0279117`. Milestone 5.7 shows incremental F8 value under adaptive selection for C-rate delta (`0.00940756`, paired p05 `6.06012e-05`), but the incremental comparison fails outside-C-rate non-degradation (`0.717391`), while reweighting-only also contributes (`0.0106361`). Milestone 5.8 routes C-rate transfer to D2 adaptive R2/F4 and non-C-rate views to D0, preserving C-rate gain `0.0106361` with paired p05 `0.00594397` and max outside-C-rate degradation `0`. Milestone 8.6 shows this does not transfer to direct `capacity_Ah_k1`; Milestone 8.7 shows capacity reconstructed from predicted delta improves C-rate rows but fails transfer because router-derived capacity has max outside-split degradation `0.293828`; Milestone 8.8 decomposes the failure and finds two failing direct-reference outside-split comparisons plus 58 degrading hotspots. | C-rate fade solved; capacity-level transfer repaired; architecture justified; adaptive gain wholly attributed to F8 stress features; global robust capacity. | Lock C22 and C24 as `delta_capacity_Ah` diagnostics; keep C35, C36, and C37 not supported and close the current capacity-level branch. |
...
| Benchmark task freeze | Supported for reproducibility | `configs/benchmark_tasks_v1.yaml` tracks the historical 19-task freeze, and `configs/benchmark_tasks_v2.yaml` adds the post-Milestone-9 T20 neural sequence architecture gate. `mbp report check-benchmark-tasks` validates task statuses against the v2 claim matrix and source artifacts. | New scientific claims; unregistered model expansion. | Use the v2 registry and leaderboard as the current benchmark interface while preserving the v1 freeze. |
...
| Neural sequence architecture gate | Not supported / CBAT blocked | Milestone 9 v2 tensor QA passed over 3,827 intervals and CUDA CNN/TCN/CNN-LSTM rows ran, but true-order sequence candidates failed the predeclared shuffled, aggregate-event HGB, timestamp-stress HGB, and C-rate `delta_capacity_Ah` controls. | Neural sequence readiness; CBAT prototype readiness; transformers; policy ranking; broad architecture claims. | Keep sequence/neural/CBAT blocked; return to synthesis or release maintenance. |
...
| Diagnostic endpoint horizon forecasting | Partially supported / selected endpoints supported for diagnostics | `diagnostic_horizon_table_v1` passes QA with 80,878 rows across 228 cells, 76 parameter-set conditions, six scalar PULSE/EIS targets, and horizons 1/2/3/5. DH3 HGB capacity plus current same-diagnostic state passes `21/24` primary horizon-2/3 10% gain rows and `22/24` C-rate non-collapse rows, so the broad endpoint gate does not fully pass. Milestone 8.2.1 forensics show `eis_z_abs_1kHz`, `nyquist_semicircle_width_proxy`, and `pulse_10ms_resistance` pass endpoint-specific diagnostic checks, while `eis_phase_1kHz`, `nyquist_im_peak_abs`, and `pulse_1s_resistance` remain partial. | Broad diagnostic endpoint forecasting; capacity+PULSE+EIS architecture; CBAT; calibrated risk or uncertainty; policy ranking; sequence/neural branches; causal or same-cell counterfactual claims. | Keep selected scalar endpoint diagnostic wording only; broad endpoint and architecture claims remain blocked. |
...
| CBAT | Blocked | Simpler gates do not justify architecture, diagnostic-state distillation is negative, calibration/risk and policy gates remain blocked, and the stronger Milestone 9 pre-CBAT neural sequence gate is negative. | CBAT validation; neural sequence readiness; transformers; policy ranking; broad architecture claims. | Do not open. |
...
supports diagnostic routing only, not a global robust model. Milestone 8.6
then audits the target boundary and shows the direct repair does not transfer
to `capacity_Ah_k1`. Milestone 8.7 tests algebraic capacity-from-delta
reconstruction; C-rate gains are positive, but the router-derived capacity path
fails outside-split non-degradation, so the C-rate repair claim remains
delta-only. Milestone 8.8 decomposes the failure by split and condition and
closes the current capacity-level reconstruction branch. No milestone
...
Milestone 9 then tests a stronger pre-CBAT neural sequence architecture gate
with v2 event tensors and CUDA CNN/TCN/CNN-LSTM rows. Tensor QA and CUDA
execution succeed, but true-order sequence candidates fail the predeclared
shuffled, aggregate-event HGB, timestamp-stress HGB, and C-rate controls, so
CBAT prototype readiness remains blocked.
...
Milestone 8.2.1 narrows that result: three scalar endpoints pass
endpoint-specific diagnostic checks, but three remain partial, so no broad
endpoint or architecture claim is authorized.
