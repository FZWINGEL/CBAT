...
| Multi-horizon capacity | Partially supported overall / C-rate and delta diagnostics supported | `capacity_horizon_table_v1` has 13,770 observed rows across 228 cells and 76 parameter sets. HGB K2 beats persistence and prior slope for C-rate horizons 2/3 on both `capacity_Ah_kh` and `delta_capacity_Ah_h`, and beats both references for all-split `delta_capacity_Ah_h` horizons 2/3. Overall capacity-level support is partial because horizon-3 `capacity_Ah_kh` HGB K2 MAE `0.0935304` is slightly worse than prior-slope `0.0932329`. | Multi-horizon forecasting solved globally; K3 oracle exposure as prospective input; architecture/policy readiness. | Lock scoped diagnostic wording only. |
