...
| Sequence models | Order-aware event features do not beat aggregate or shuffled controls, and the Milestone 7.1 fixed-length event-sequence CUDA Torch MLP reopening check does not beat aggregate-event HGB, timestamp-stress HGB, or C-rate delta controls. | `docs/experiments/2026-05-23_temporal_history_value_gate.md`; `docs/experiments/2026-05-27_minimal_sequence_reopening_gate.md` |
| CBAT | Simpler baselines, calibration, sequence, and multimodal gates do not justify architecture. | `docs/PROJECT_CHARTER.md` |
| Policy ranking | Milestone 7.2 shows observed matched contrast support and sign stability. Milestone 7.3 adds partial existing-prediction contrast-ordering signal, but HGB K2 fails the strict prior-slope bootstrap reference gate. Milestone 7.4 decomposes that failure by effect size and rank metric, but the result remains diagnostic-only and there is still no calibrated risk, intervention test, or causal evidence. | `reports/analysis/policy/policy_claim_readiness.md`; `reports/analysis/policy/policy_ranking_claim_readiness.md`; `reports/analysis/policy/policy_ranking_failure_claim_readiness.md`; `reports/baselines/threshold_warning_calibration/threshold_warning_calibration_claim_readiness.md` |
| Policy recommendation | Observed degradation ordering is not an optimization, intervention, safety, or deployment policy. | `reports/analysis/policy/policy_claim_readiness.md` |
| Deployment or support-filtered reliability | Milestone 8.0 support filtering is an audit/abstention diagnostic only. At 50% retention, primary capacity and threshold-warning metrics worsen, and C-rate support reliability is not supported. | `reports/analysis/support_reliability/support_reliability_claim_readiness.md` |
