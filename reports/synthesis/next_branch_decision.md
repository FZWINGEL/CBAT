...
Milestones 8.4 through 8.7 then revisit the most persistent capacity blocker:
...
over-broadening: the same direct repair does not transfer to `capacity_Ah_k1`.
The reconstruction audit adds that `capacity_Ah_k + predicted_delta` improves
the C-rate capacity row, but the router-derived capacity path fails the
outside-split guardrail. Two-target repair, broad robust capacity, and solved
C-rate fade remain unsupported.
...
7. Do not broaden the Milestone 8.5 C-rate repair claim. Milestones 8.6 and
   8.7 show the adaptive/router repair remains a `delta_capacity_Ah`
   diagnostic: direct capacity transfer fails, and capacity-from-delta
   reconstruction fails the outside-split transfer guardrail.
...
Milestone 8.6 supplies the direct target boundary check and keeps the result
narrow: adaptive R2/F8 worsens C-rate `capacity_Ah_k1` versus both F4 and
