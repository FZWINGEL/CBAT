# Benchmark v0.1-rc1 Release Notes

Release title: Benchmark v0.1 release candidate for grouped-validation battery
...
- EIS QA, scalar feature readiness, and scalar diagnostic baselines
- semi-empirical comparator reports
- replicate uncertainty and grouped calibration diagnostics
...
- v2 claim ledger, claim matrix, gate status, blocked-claim register, and next
  branch decision
- benchmark reproducibility docs, runbook, command DAG, artifact manifest, and
  release checks
...
- raw HGB quantiles or conformal intervals validate global calibrated
  uncertainty
...
- The command DAG is documented and checked for command coverage, but it is not
  a workflow executor.
- Calibrated risk and calibrated uncertainty remain blocked.
