...
| C10 | EIS improves anything. | `gated` | EIS QA, valid-frequency masks, and grouped predictive evidence. | Milestone 2.1 shows scalar EIS endpoints are usable for diagnostics and prior-EIS features have split-dependent signal, but paired strongest-non-EIS evidence and alignment robustness are still pending. | EIS is a gated scalar diagnostic modality; improvement claims remain blocked pending stronger grouped comparisons. | EIS improves degradation prediction. | `docs/experiments/2026-05-23_eis_scalar_diagnostic_baselines.md` |
