...
|---|---|---|---|---|---|---|---|
| C01 | Actual LOG_AGE features improve over simple state/time baselines. | `partially_supported` | Grouped split gains beyond state/time and nominal protocol. | HGB F3 to F4 mean gain is `0.000085`, median `0.001124`, positive rows `10 / 20`; best focused HGB rows often use F4. | Current scalar LOG_AGE features help nonlinear models in some grouped views, but the effect is mixed. | LOG_AGE scalar features broadly solve operating-history generalization. | `docs/experiments/2026-05-22_capacity_baseline_synthesis.md` |
...
| C10 | EIS improves anything. | `gated` | EIS QA, valid-frequency masks, and grouped predictive evidence. | EIS result-data products exist, but EIS QA and predictive tests are pending. | EIS is a future gated modality. | EIS improves degradation prediction. | `docs/REPO_STATUS.md` |
