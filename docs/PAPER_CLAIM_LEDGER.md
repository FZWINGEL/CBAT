...
| C09 | Quantile HGB uncertainty is calibrated. | `not_supported` | q10-q90 coverage close to nominal 0.8 across grouped splits. | Milestone 5.2 noncrossing quantile hygiene does not fix undercoverage: raw q10/q90 mean coverage is `0.701398`, minimum split/fold coverage is `0.00226244`, and C-rate coverage remains unacceptable. | Raw HGB quantile intervals are undercovered and remain diagnostics only. | HGB quantile intervals are calibrated uncertainty estimates. | `reports/analysis/calibration_capacity/calibration_claim_readiness.md` |
