...
| KI006 | LOG_AGE timestamp/EFC monotonicity violations | Reduced LOG_AGE table and 1,054 interval rows | Medium | `policy_defined` | Strict QA detects 7,107 timestamp/EFC decreases; `reports/audit/log_age_monotonicity_violations.parquet` records 7,071 default-tolerance detailed violations, all EFC-only in the summary sample, and `interval_qa_report.json` confirms 1,054 affected intervals are quality-flagged. `docs/LOG_AGE_MONOTONICITY_POLICY.md` defines strict and tolerant baseline subsets; Milestone 0.5 baselines must report sensitivity excluding flagged intervals. |
