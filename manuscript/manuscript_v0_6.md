...
This venue-neutral draft converts the v0.5 evidence integration into a
reviewer-ready benchmark manuscript. It uses the validated
`benchmark-v0.1-rc2` release package as its reproducibility anchor and keeps
claim-control details in `manuscript/manuscript_v0_6_traceability.md`.

## Abstract

Battery degradation benchmarks can overstate model readiness when train and
test data share operating-condition structure. We present a reproducible
...
with three replicate cells each, and headline evidence is reported under
condition-level and stressor-axis holdouts rather than random row splits.
...
falsification, and knee-label stability tests all constrain the final claim
posture. The strongest late-stage positive result is narrow: non-neural
baselines forecast the 80% capacity-relative threshold event diagnostically,
beyond proximity baselines and under verified-only censoring sensitivity.

The contribution is a claim-bounded benchmark release candidate, not a new
architecture. The `benchmark-v0.1-rc2` package includes the runbook, command
DAG, artifact manifest, release checker, claim ledger, and source-linked
synthesis reports needed to audit and reproduce the evidence.

## Introduction

Battery degradation depends on realized stress history, not only on nominal
protocol labels. Temperature, voltage window, state of charge, current profile,
C-rate, rest, check-up burden, and prior age interact over time. A benchmark
for this setting should therefore test generalization to held-out operating
families and should separate supported evidence from tempting but unsupported
modeling branches.

The Luh-Blank dataset is useful for this purpose because it combines capacity
check-ups, LOG_AGE operating histories, PULSE resistance, and EIS records for
228 commercial cells. The experimental design is 76 parameter-set conditions
with three replicate cells each, so the benchmark treats condition triplets as
the primary grouping unit. Random splits are retained only as leakage smoke
tests, not as paper-facing evidence.

This study asks which signals survive grouped validation and which do not. It
builds versioned data products, evaluates scalar baselines, introduces
diagnostic modalities only after QA gates, challenges flexible models with
domain comparators, quantifies replicate and interval reliability limitations,
and records negative gates before any architecture work. Figure 1 summarizes
the linked data-product architecture, and Figure 2 summarizes the grouped
validation design.
...
and ignored; tracked reports, manifests, and release checks carry the
reviewable evidence.
...
targets, RT/50 EIS features and targets, run-event/sequence summaries, knee and
threshold labels, and threshold-warning tables. Table 1 summarizes the audited
cohort and interval population. Table 8 summarizes the release artifact policy.
...
The focused F4 C-rate holdout condition-mean MAE is 0.125186 for
`capacity_Ah_k1` and 0.101133 for `delta_capacity_Ah`.

LOG_AGE scalar summaries and stress features help in selected grouped views,
but the gains are mixed. Stress-feature v1.1 improves the C-rate capacity-level
row from 0.125186 to 0.120605, while C-rate `delta_capacity_Ah` remains worse
than the F4 threshold at 0.102516. Normalized rate targets, train-fold bias
...
The charter requires interpretable comparators before architecture work.
Semi-empirical ridge-style stress baselines provide that domain check. They do
not beat the strongest grouped HGB/stress baselines in the hardest C-rate
capacity and fade views, but they are useful reference points for interpreting
flexible ML baselines.
...
turning model intervals into validated deployment-style reliability.
...
coverage gate. Global interval-reliability wording therefore remains blocked.
Figure 12 summarizes the replicate and interval-diagnostic gate, and Table 6
places this result in the full gate matrix.
...
aggregate exposure. Milestone 2.4 built a streamed LOG_AGE run-event table and
...
view. Sequence models remain blocked.
...
and synthesis reports. It excludes raw data and generated Parquets. The v0.6
manuscript package sits on top of that release: it changes presentation and
traceability, not the scientific evidence.
...
Semi-empirical and replicate-aware comparators strengthen the benchmark by
forcing flexible ML results to compete against interpretable references.
Interval diagnostics, temporal-order falsification, and knee-label stability
tests prevent unsupported uncertainty, sequence, and knee-prediction claims.
Threshold-event forecasting is the strongest late-stage positive result, but it
remains diagnostic and support-bounded.
